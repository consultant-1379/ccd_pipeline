pipeline {
  ...
  stages {
    ...
    stage('Run Image Check on XDN SFTP') {
      steps {
        withCredentials([usernamePassword(credentialsId: 'xdn_sftp', passwordVariable: 'SFTP_PASSWORD', usernameVariable: 'SFTP_USER')]) {
          sh "cd ccd_pipeline/ansible; ansible-playbook -e deployment_id=${deployment_id} -e sftp_user=${SFTP_USER} -e sftp_password=${SFTP_PASSWORD} check-and-upload-images-in-xdn.yml -vvv" 
        }
      }
    }
    ...
    stage('Run Generate Config for deployment') {
      steps {
        withCredentials([usernamePassword(credentialsId: 'xdn_sftp', passwordVariable: 'SFTP_PASSWORD', usernameVariable: 'SFTP_USER')]) {
          sh "cd ccd_pipeline/ansible; ansible-playbook -e deployment_id=${deployment_id} -e sftp_user=${SFTP_USER} -e sftp_password=${SFTP_PASSWORD} generate-and-upload-conf-in-xdn.yml -vvv"
        }
      }
    }
