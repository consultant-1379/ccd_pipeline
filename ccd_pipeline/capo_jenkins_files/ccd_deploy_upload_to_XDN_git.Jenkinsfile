...
def getFromSftp(String user,String password,String host,String dir ){
  sh"""
  lftp -u $user,$password -e 'set sftp:connect-program "ssh -o StrictHostKeyChecking=no";get ccd_pipeline/repo/capo_repo.tgz -o $dir/capo_repo.tgz;exit' sftp://$host
  """
}
...
list=\$(ls /tmp/minio/managed-config/ | sed 's/\\(.*\\)\\.env\\.yml/\\1/; s/\\(.*\\)\\.template\\.yml/\\1/; s/\\(.*\\)\\.yml/\\1/' | uniq)
...
// HERE RMOE XDN_CAPO checkout when ready git checkout XDN_CAPO
def unpackAndPush(String repo, String user,String password){
    sh"""cd ccd-pipeline
        git config --global user.name "Jenkins User"
        git config --global user.email "jenkins@example.com"
        tar xvf capo_repo.tgz
        rm capo_repo.tgz
        git add .
        if git diff --cached --exit-code; then
            echo "No changes to commit."
        else
            git commit -m "UPDATE from CAPO build: ${env.BUILD_NUMBER}"
